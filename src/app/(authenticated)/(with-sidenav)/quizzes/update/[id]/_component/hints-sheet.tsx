...
function HintsSheet(props: Props) {
    ...
    const handleDeleteHint = (localId: string) => {
        const hint = props.hints.find((h) => localId === h.localId)
        if (!hint?.isNew && hint?.id) {
            toastLoading("Deleting the hint...")
            deleteHintById(hint.id)
                .then(() => {
                    toastSuccess("Hint deleted successfully.")
                    props.setHints(
                        props.hints.filter((hint) => hint.localId !== localId)
                    )
                })
                .catch(() => toastError("Something went wrong..."))
                .finally(() => dismissToasts("loading"))
        } else {
            props.setHints(
                props.hints.filter((hint) => hint.localId !== localId)
            )
    ...
    const selectedHint = useMemo(
        () =>
            props.hints.find((hint) => String(hint.id) === String(selectedId)),
