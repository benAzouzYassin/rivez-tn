...
class FontFaceObject {
    ...
    createFontFaceRule() {
        ...
        const url = `url(data:${this.mimetype};base64,${toBase64Util(
            this.data
        )});`;
        let rule;
        if (!this.cssFontInfo) {
    ...
    setFillAndStrokeStyleToContext(graphics, paintType, color) {
        ...
        switch (paintType) {
            ...
            case PaintType.UNCOLORED:
                const cssColor = Util.makeHexColor(
                    color[0],
                    color[1],
                    color[2]
                );
        ...
        return shadow(
            ...
            new Map([
                ...
                [
                    "monospace",
                    `${
                        isWindows && isFirefox ? "Lucida Console, " : ""
                    }monospace`,
                ],
            ])
        );
...
class InkAnnotationElement extends AnnotationElement {
    ...
    #getTransform(rotation, rect) {
        switch (rotation) {
            case 90:
                return {
                    transform: `rotate(90) translate(${-rect[0]},${
                        rect[1]
                    }) scale(1,-1)`,
                    width: rect[3] - rect[1],
    ...
    toSVGPath() {
        ...
        for (let i = 6; i < top.length; i += 6) {
            if (isNaN(top[i])) {
                ...
                buffer.push(
                    `C${top[i]} ${top[i + 1]} ${top[i + 2]} ${top[i + 3]} ${
                        top[i + 4]
                    } ${top[i + 5]}`
                );
            }
        ...
        for (let i = bottom.length - 6; i >= 6; i -= 6) {
            if (isNaN(bottom[i])) {
                ...
                buffer.push(
                    `C${bottom[i]} ${bottom[i + 1]} ${bottom[i + 2]} ${
                        bottom[i + 3]
                    } ${bottom[i + 4]} ${bottom[i + 5]}`
                );
            }
        }
...
class FreeDrawOutline extends Outline {
    ...
    toSVGPath() {
        ...
        for (let i = 6, ii = this.#outline.length; i < ii; i += 6) {
            ...
            buffer.push(
                `C${this.#outline[i]} ${this.#outline[i + 1]} ${
                    this.#outline[i + 2]
                } ${this.#outline[i + 3]} ${this.#outline[i + 4]} ${
                    this.#outline[i + 5]
                }`
            );
        }
    ...
    static startDrawing(parent, uiManager, _isLTR, event) {
        ...
        window.addEventListener(
            ...
            (e) => {
                ...
                (DrawingEditor.#currentPointerIds ||= new Set()).add(
                    e.pointerId
                );
...
class InkDrawOutline extends Outline {
    ...
    toSVGPath() {
        ...
        for (const { line } of this.#lines) {
            ...
            if (line.length === 12) {
                buffer.push(
                    `L${Outline.svgRound(line[10])} ${Outline.svgRound(
                        line[11]
                    )}`
                );
                continue;
    ...
    get defaultProperties() {
        ...
        return {
            ...
            path: {
                "transform-origin": `${Outline.svgRound(x)} ${Outline.svgRound(
                    y
                )}`,
            },
        };
    ...
    getPathResizingSVGProperties([newX, newY, newWidth, newHeight]) {
        ...
        if (
            ...
            return {
                path: {
                    "transform-origin": `${Outline.svgRound(
                        newX
                    )} ${Outline.svgRound(newY)}`,
